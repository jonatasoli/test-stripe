            ...
            <transition-group name="list" tag="div">
              ...
              <div v-for="item in filteredData" :key="item.id" class="flex-table-item">
                ...
                <div class="flex-table-cell" data-th="Status">
                  <span v-if="item.status === 'Online'" class="tag is-success is-rounded">{{ item.status }}</span>
                  <span v-if="item.status === 'Working'" class="tag is-info is-rounded">{{ item.status }}</span>
                  <span v-if="item.status === 'Suspended'" class="tag is-danger is-rounded">{{ item.status }}</span>
                  <span v-if="item.status === 'Offline'" class="tag is-rounded">{{ item.status }}</span>
                </div>
                <div class="flex-table-cell" data-th="Relations">
                  <VAvatarStack :avatars="item.contacts" size="small" :limit="3" class="is-pushed-mobile" />
                </div>
